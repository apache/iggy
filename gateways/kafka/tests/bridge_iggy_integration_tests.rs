...
/// `ensure_stream_and_topic` is reachable directly through the public `TopicCatalog` trait, not
/// only through `CreateTopics`' own wire-validation layer - a caller that bypasses that layer
/// (a test, or a future auto-create path) must not be able to provision a topic nothing can
/// produce to.
#[tokio::test]
#[serial]
async fn ensure_stream_and_topic_rejects_a_zero_partition_count() {
    let data_dir = tempfile::tempdir().expect("tempdir");
    let server = TestServer::spawn(data_dir.path()).await;
    let bridge = IggyBridge::connect(server.test_config())
        .await
    ...
    let err = bridge
        ...
        .await
        .expect_err("partition_count == 0 must be rejected, not silently create an unusable topic");

    assert!(
        matches!(err, BridgeError::InvalidPartitionCount { .. }),
        "expected InvalidPartitionCount, got {err:?}"
    );
    assert_eq!(
        err.to_kafka_error_code(),
