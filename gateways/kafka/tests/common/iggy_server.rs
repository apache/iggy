...
//! Real `iggy-server` process spawn helper - compiled into each integration test binary via
//! `#[path]`. Shared by `bridge_iggy_integration_tests.rs` (exercises `IggyBridge` directly) and
//! `gateway_bridge_e2e_tests.rs` (exercises `KafkaGateway` + a real `IggyBridge` over the Kafka
//! wire) - extracted here rather than duplicated once a second suite needed the identical
//! spawn/port-lock/graceful-shutdown machinery.
#![allow(dead_code)]
