...
function load_async_dependencies(): void
{
    if (class_exists('\\Revolt\\EventLoop') && function_exists('\\Amp\\async')) {
        return;
    }

    $autoload = dirname(__DIR__) . '/vendor/autoload.php';
    if (!is_file($autoload)) {
        throw new SkippedTest('run composer install to install Revolt/Amp async test dependencies');
    }

    require_once $autoload;

    if (!class_exists('\\Revolt\\EventLoop') || !function_exists('\\Amp\\async')) {
        throw new SkippedTest('Revolt/Amp async test dependencies are unavailable');
    }
}

function run_async(callable $callback): mixed
{
    ...
    if ($stream === false) {
        throw new TestFailure('failed to open php-tokio readiness fd');
    }
    stream_set_blocking($stream, false);

    $watcher = \Revolt\EventLoop::onReadable(
        $stream,
        static fn (): null => IggyAsyncClient::wakeup(),
    );
