...
func TestDiagnosticEvents_LoginEmitsSignedIn(t *testing.T) {
	...
	_, err := cli.LoginUser(context.Background(), rootUsername, rootPassword)
...
// emitted across connect → login → logout → close on a single subscriber.
func TestDiagnosticEvents_FullLifecycle(t *testing.T) {
	address := serverAddress(t)

	cli, err := client.NewIggyClient(client.WithTcp(tcp.WithServerAddress(address)))
