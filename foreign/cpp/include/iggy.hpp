...
public:
  ...
  static Expiry never_expire() {
    return Expiry("never_expire", std::numeric_limits<std::uint64_t>::max());
  }
...
public:
  ...
  std::string_view polling_strategy_kind() const {
    return polling_strategy_kind_;
  }
