...
TEST_F(E2E_Stream, CreateStreamWithEmojiName) {
    ...
    TrackStream(stream_name);
    ASSERT_NO_THROW({
        const auto stream_details              = ffi_client->get_stream(make_string_identifier(stream_name));
...
TEST_F(E2E_Stream, UpdateStreamWorksCorrectly) {
    ...
    ASSERT_NO_THROW({ original_stream_details = ffi_client->get_stream(make_string_identifier(stream_name)); });
    const std::uint32_t stream_id = original_stream_details.id;
    ForgetTrackedStream(stream_name);
    TrackStream(stream_id);

    ASSERT_NO_THROW(
        client.UpdateStream(iggy::Identifier::String(stream_name), updated_stream_name));

    ASSERT_THROW(ffi_client->get_stream(make_string_identifier(stream_name)), std::exception);

    iggy::ffi::StreamDetails updated_stream_details{};
    ASSERT_NO_THROW({ updated_stream_details = ffi_client->get_stream(make_numeric_identifier(stream_id)); });
...
TEST_F(E2E_Stream, UpdateStreamWithSameNameIsIdempotent) {
    ...
    ASSERT_NO_THROW({ first_read = ffi_client->get_stream(make_string_identifier(stream_name)); });
    ASSERT_NO_THROW(client.UpdateStream(iggy::Identifier::Numeric(first_read.id), stream_name));
    ASSERT_NO_THROW({ second_read = ffi_client->get_stream(make_numeric_identifier(first_read.id)); });
...
TEST_F(E2E_Stream, UpdateStreamWithVariousUtf8Characters) {
    ...
    for (const auto &updated_stream_name : updated_stream_names) {
        ...
        ASSERT_NO_THROW(
            client.UpdateStream(iggy::Identifier::Numeric(stream_id), updated_stream_name));
        ASSERT_NO_THROW({
            const auto stream_details = ffi_client->get_stream(make_numeric_identifier(stream_id));
...
TEST_F(E2E_Stream, UpdateNonExistentStreamThrows) {
    ...
    const std::string stream_name         = GetRandomName();
    const std::string updated_stream_name = GetRandomName();
    ASSERT_THROW(client.UpdateStream(iggy::Identifier::String(stream_name), updated_stream_name),
                 std::exception);
}
...
TEST_F(E2E_Stream, UpdateStreamWithDuplicateNameThrows) {
    ...
    ASSERT_THROW(
        client.UpdateStream(iggy::Identifier::String(first_stream_name), second_stream_name),
        std::exception);
}
...
TEST_F(E2E_Stream, UpdateStreamFailedValidationDoesNotMutateStream) {
    ...
    ASSERT_NO_THROW({ stream_before_failed_update = ffi_client->get_stream(make_string_identifier(stream_name)); });
    ...
    ASSERT_NO_THROW({ stream_after_failed_update = ffi_client->get_stream(make_string_identifier(stream_name)); });
    ...
    ASSERT_THROW(ffi_client->get_stream(make_string_identifier(updated_stream_name)), std::exception);
}
...
TEST_F(E2E_Stream, UpdateStreamOnlyChangesName) {
    ...
    ASSERT_NO_THROW(ffi_client->create_topic(make_numeric_identifier(stream_id), topic_name, 2, "none", "never_expire", 0,
                                         "server_default", {}));
    ...
    ASSERT_NO_THROW({
        stream_before_update = ffi_client->get_stream(make_numeric_identifier(stream_id));
        stats_before_update  = ffi_client->get_stats();
    });
    ...
    ASSERT_THROW(ffi_client->get_stream(make_string_identifier(stream_name)), std::exception);
    ASSERT_NO_THROW({
        stream_after_update = ffi_client->get_stream(make_numeric_identifier(stream_id));
        stats_after_update  = ffi_client->get_stats();
    });
...
TEST_F(E2E_Stream, UpdateStreamValidatesNameBounds) {
    ...
    for (const auto &invalid_stream_name : invalid_stream_names) {
        ...
        ASSERT_THROW(
            client.UpdateStream(iggy::Identifier::Numeric(stream_id), invalid_stream_name),
            std::exception);
    }
}
...
TEST_F(E2E_Stream, DeleteStreamWithInvalidIdentifierThrows) {
...
TEST_F(E2E_Stream, GetStreamDetailsWithInvalidIdentifierThrows) {
...
TEST_F(E2E_Stream, GetStreamByNumericIdentifierReturnsStreamDetails) {
    auto client = GetLoggedInHighLevelClient();
    ...
    std::uint32_t stream_id = 0;
    ASSERT_NO_THROW({
        const auto by_string = client.GetStream(iggy::Identifier::String(stream_name));
        stream_id            = by_string.Id();
    });

    ASSERT_NO_THROW({
        const auto by_numeric = client.GetStream(iggy::Identifier::Numeric(stream_id));
        EXPECT_EQ(by_numeric.Id(), stream_id);
        EXPECT_EQ(by_numeric.Name(), stream_name);
        EXPECT_EQ(by_numeric.TopicsCount(), 0u);
...
TEST_F(E2E_Stream, GetStreamsFieldsVerification) {
    ...
    const std::string topic_name = GetRandomName();
    ffi_client->create_topic(make_numeric_identifier(stream.Id()), topic_name, 1, "none", "never_expire", 0,
                         "server_default", {});
...
TEST_F(E2E_Stream, PurgeStreamWithInvalidIdentifierThrows) {
...
TEST_F(E2E_Stream, PurgeStreamPreservesStreamMetadata) {
    ...
    ASSERT_NO_THROW(client.CreateStream(stream_name));
    TrackStream(stream_name);
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), first_topic_name, 2, "gzip", "duration",
                                         1000, "1GiB", {}));
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), second_topic_name, 3, "none",
                                         "never_expire", 0, "server_default", {}));

    const auto stream_before_purge = ffi_client->get_stream(make_string_identifier(stream_name));
    ...
    const auto stream_with_messages = ffi_client->get_stream(make_string_identifier(stream_name));
    ...
    const auto stream_after_purge = ffi_client->get_stream(make_string_identifier(stream_name));
...
TEST_F(E2E_Stream, PurgeStreamRemovesMessagesAndPreservesTopics) {
    ...
    ASSERT_NO_THROW(client.CreateStream(stream_name));
    TrackStream(stream_name);
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), first_topic_name, 1, "none",
                                         "server_default", 0, "server_default", {}));
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), second_topic_name, 1, "none",
                                         "server_default", 0, "server_default", {}));

    const auto created_stream = ffi_client->get_stream(make_string_identifier(stream_name));
    ...
    const auto streams_before_purge = ffi_client->get_streams();
    ...
    const auto streams_after_purge = ffi_client->get_streams();
...
TEST_F(E2E_Stream, PurgeStreamAcrossMultipleTopicsAndPartitionsClearsEverything) {
    ...
    ASSERT_NO_THROW(client.CreateStream(stream_name));
    TrackStream(stream_name);
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), first_topic_name, 2, "none",
                                         "server_default", 0, "server_default", {}));
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), second_topic_name, 3, "none",
                                         "server_default", 0, "server_default", {}));
...
TEST_F(E2E_Stream, PurgeStreamThenSendMessagesAgainSucceeds) {
    ...
    ASSERT_NO_THROW(client.CreateStream(stream_name));
    TrackStream(stream_name);
    ASSERT_NO_THROW(ffi_client->create_topic(make_string_identifier(stream_name), topic_name, 1, "none", "server_default",
                                         0, "server_default", {}));
    ...
    ASSERT_NO_THROW(ffi_client->send_messages(make_numeric_identifier(created_stream.id), make_numeric_identifier(topic_id),
    ...
    const auto stream_after_resend = ffi_client->get_stream(make_string_identifier(stream_name));
...
TEST_F(E2E_Stream, PurgeStreamTwiceKeepsStreamEmptyAndTopicsIntact) {
    ...
    ASSERT_NO_THROW(client.PurgeStream(iggy::Identifier::String(stream_name)));
    const auto stream_after_first_purge = ffi_client->get_stream(make_string_identifier(stream_name));
    ...
    ASSERT_NO_THROW(client.PurgeStream(iggy::Identifier::String(stream_name)));
    const auto stream_after_second_purge = ffi_client->get_stream(make_string_identifier(stream_name));
