...
use crate::http::http_refresh_token::RefreshToken;
use crate::http::http_transport::HttpTransport;
use async_broadcast::{broadcast, Receiver, Sender};
use async_trait::async_trait;
use iggy_common::locking::{IggySharedMut, IggySharedMutFn};
use iggy_common::{DiagnosticEvent, HttpClientConfig, IdentityInfo, IggyDuration, IggyError};
...
impl Default for HttpClient {
    fn default() -> Self {
        HttpClient::create(Arc::new(HttpClientConfig::default()), Vec::new()).unwrap()
    }
}

impl HttpClient {
    /// Create a new HTTP client for interacting with the Iggy API using the provided API URL.
    pub fn new(api_url: &str, valid_paths: Vec<String>) -> Result<Self, IggyError> {
        Self::create(
            Arc::new(HttpClientConfig {
...
impl HttpClient {
    ...
    async fn fail_if_not_authenticated(&self, path: &str) -> Result<(), IggyError> {
        if self.valid_paths.contains(&path.to_string()) {
            return Ok(());
        }
...
impl HttpTransport for HttpClient {
    ...
    async fn refresh_access_token(&self) -> Result<(), IggyError> {
        ...
        let command = RefreshToken::new(token.to_owned());
