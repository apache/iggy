...
pub use kit::{Setup, TopicDefaults};
pub use lane_builder::lane;
...
impl NamespacePrefixes {
    ...
    pub fn create_prefix(&self, ns: TargetNamespace) -> &str {
        match ns {
            TargetNamespace::Stable => &self.stable,
            TargetNamespace::Churn | TargetNamespace::Both => &self.churn,
        }
    }
}
...
pub fn create_scenario(name: ScenarioName) -> Box<dyn Scenario> {
    match name {
        ...
        ScenarioName::ConsumerGroupChurn => Box::new(consumer_group_churn::ConsumerGroupChurn),
    }
}
...
pub fn list_scenarios() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            consumer_group_churn::ConsumerGroupChurn.name(),
            consumer_group_churn::ConsumerGroupChurn.describe(),
        ),
    ]
}
