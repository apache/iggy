...
impl OpOutcome {
    pub fn success_from(detail: Option<OpDetail>) -> Self {
        Self::Success { detail }
    }
...
impl Op {
    ...
    /// Resource key for ops that destroy resource visibility (delete/purge).
    /// Used by `InFlightOps` to bridge the TOCTOU gap between server execution
    /// and shadow state update.
    pub fn in_flight_key(&self) -> Option<String> {
        match self {
            Self::DeleteStream { name } | Self::PurgeStream { name } => Some(name.clone()),
            Self::DeleteTopic { stream, topic } | Self::PurgeTopic { stream, topic } => {
                Some(format!("{stream}/{topic}"))
            }
            _ => None,
        }
    }
