...
use crate::scenarios::{Namespace, NamespacePrefixes, OriginScope, TargetNamespace};
use rand::{Rng, RngExt};
...
const TOMBSTONE_RETENTION: Duration = Duration::from_secs(30);

pub struct ShadowState {
    pub streams: HashMap<String, ShadowStream>,
...
pub struct ShadowStream {
    pub topics: HashMap<String, ShadowTopic>,
...
pub struct ShadowTopic {
    pub partitions: u32,
    pub consumer_groups: HashSet<String>,
    pub origin: ResourceOrigin,
}
...
fn matches_scope(origin: ResourceOrigin, scope: OriginScope) -> bool {
    match scope {
        OriginScope::Any => true,
        OriginScope::SetupOnly => origin == ResourceOrigin::Setup,
        OriginScope::DynamicOnly => origin == ResourceOrigin::Dynamic,
    }
}

fn matches_namespace(ns: Namespace, target_ns: TargetNamespace) -> bool {
    match target_ns {
        TargetNamespace::Stable => ns == Namespace::Stable,
        TargetNamespace::Churn => ns == Namespace::Churn,
        TargetNamespace::Both => true,
    }
}
