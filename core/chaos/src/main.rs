...
async fn main() -> Result<(), LabError> {
    ...
    match args.command {
        ...
        Command::Viz(viz_args) => match perfetto::convert(&viz_args.bundle_dir) {
            Ok(stats) => {
                let out = viz_args.bundle_dir.join("trace.perfetto.json");
                println!(
                    "Wrote {} events to {} ({} unmatched outcomes)",
                    stats.events,
                    out.display(),
            ...
            Err(e) => {
                eprintln!("Failed to convert trace: {e}");
                std::process::exit(1);
            }
        },
