...
use crate::ops::{ErrorClass, Op, OpDetail, OpOutcome};
...
impl LabReplay {
    ...
    pub async fn run(self) -> Result<ReplayOutcome, LabError> {
        ...
        let mut payload_gen = PayloadGenerator::new(0, self.args.message_size);
        ...
        for intent in &intents {
            let replay_outcome = match intent
                .op
                .execute(&client, &mut payload_gen, &prefixes)
                .await
            {
    ...
    fn load_traces(&self) -> Result<(Vec<ReplayIntent>, HashMap<u64, OriginalOutcome>), LabError> {
        ...
        // Try single trace.jsonl first (new format), fall back to per-worker files
        let trace_files = {
            let single = trace_dir.join("trace.jsonl");
            if single.exists() {
                vec![single]
            } else {
