 ...
 * under the License.
 */
use crate::actors::producer::backend::{BenchmarkProducerBackend, HighLevelBackend, ProducedBatch};
use iggy::prelude::*;
use integration::test_server::login_root;
use tokio::time::Instant;
use tracing::info;

impl BenchmarkProducerBackend for HighLevelBackend {
    ...
    fn log_setup_info(&self) {
        info!(
            "Producer #{} → sending {} messages per batch (each {} bytes) to stream {} across {} partition(s), using high-level API...",
