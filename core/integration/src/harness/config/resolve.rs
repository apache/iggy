...
fn find_similar_paths(unknown: &str) -> Vec<String> {
    // Normalize: replace underscores with dots for comparison
    ...
    let mut candidates: Vec<_> = ServerConfig::env_mappings()
        ...
        .filter_map(|m| {
            ...
            // Check if the path ends with the unknown value (suffix match)
