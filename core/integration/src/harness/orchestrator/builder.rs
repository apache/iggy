...
fn default_cluster_node_count() -> usize {
    ...
    // against an N-node cluster (e.g. `IGGY_TEST_CLUSTER_NODES=3` probes the
    // whole vsr suite on a 3-node cluster). Explicit attrs win.
    std::env::var("IGGY_TEST_CLUSTER_NODES")
        .ok()
