...
impl std::fmt::Display for TestBinaryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TestBinaryError::DirectoryCreation { path, source } => {
            ...
            TestBinaryError::MissingConnector => {
                write!(f, "Connector runtime not configured in harness")
            }
...
impl std::error::Error for TestBinaryError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            TestBinaryError::DirectoryCreation { source, .. } => Some(source),
            _ => None,
        }
    }
}
