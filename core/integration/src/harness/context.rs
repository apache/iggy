...
impl TestContext {
    ...
    pub fn ensure_created(&mut self) -> Result<(), TestBinaryError> {
        ...
        fs::create_dir_all(&self.base_dir).map_err(|e| TestBinaryError::DirectoryCreation {
    ...
    pub fn connector_stdout_path(&self) -> PathBuf {
        self.base_dir.join("connectors_stdout.log")
    }

    pub fn connector_stderr_path(&self) -> PathBuf {
...
mod tests {
    ...
    fn test_context_paths() {
        ...
        assert!(ctx.connector_state_path().ends_with("connectors_state"));
    }
}
