...
/// Publishes JSON messages and asserts each becomes a DAG-run POST with conf + dag_run_id.
#[iggy_harness(
    server(connectors_runtime(config_path = "tests/connectors/airflow/sink.toml")),
    seed = seeds::connector_stream
)]
async fn given_json_messages_when_consumed_should_post_dag_runs(
    harness: &TestHarness,
    ...
    let requests = fixture
        .container()
        .wait_for_dag_run_requests(TEST_MESSAGE_COUNT)
        .await
        .expect("WireMock did not receive expected DAG-run POSTs");

    assert_eq!(requests.len(), TEST_MESSAGE_COUNT);
    ...
    for req in &requests {
        ...
        assert!(
            ...
            "missing deterministic dag_run_id: {body}"
        );
        let conf = body
            .get("conf")
            ...
            .expect("conf object");
        assert!(
            conf.contains_key("order_id"),
            "conf should keep payload: {body}"
        );
        assert!(
