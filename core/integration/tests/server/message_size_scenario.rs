...
/// around the maximum message size. When run on other transports, it will fail
/// because both QUIC and HTTP have different message size limits.
...
async fn message_size_scenario(harness: &TestHarness) {
    ...
    // Send message and check the result
...
async fn assert_clean_system(client: &IggyClient) {
    let stats = client.get_stats().await.unwrap();
    assert_eq!(stats.streams_count, 0);
