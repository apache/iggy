...
#[iggy_harness(
    transport = Tcp,
    server(
        system.partition.messages_required_to_save = 100000,
        system.partition.size_of_messages_required_to_save = "1GB",
        system.partition.enforce_fsync = false,
        message_saver.enabled = true,
        message_saver.interval = "100ms"
    )
)]
async fn should_read_messages_during_background_saver_flush(harness: &TestHarness) {
    ...
    let consumer = harness.tcp_root_client().await.unwrap();
