...
async fn should_handle_stale_client_with_manual_reconnection(harness: &TestHarness) {
    let server_addr = harness.server().tcp_addr().unwrap().to_string();
...
async fn should_handle_stale_client_with_auto_reconnection(harness: &TestHarness) {
    let server_addr = harness.server().tcp_addr().unwrap().to_string();
