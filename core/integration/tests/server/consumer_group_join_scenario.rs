...
use crate::server::{
    ...
    USERNAME_3, cleanup, join_consumer_group,
};
...
use integration::harness::{assert_clean_system, create_user, login_user};
use integration::iggy_harness;

#[iggy_harness(transport = [Tcp, WebSocket, Quic], server(tcp.socket.nodelay = true))]
async fn matrix(harness: &TestHarness) {
    let system_client = harness.root_client().await.unwrap();
