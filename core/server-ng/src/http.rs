...
use configs::cluster::{ClusterConfig, ClusterNodeConfig, TransportPorts};
use configs::http::HttpConfig;
use consensus::{CLIENTS_TABLE_MAX, MetadataHandle, VsrConsensus};
...
use iggy_binary_protocol::responses::personal_access_tokens::{
    GetPersonalAccessTokensResponse, RawPersonalAccessTokenResponse,
};
...
// `DEFAULT_ROOT_USER_ID` is only referenced by the test module now that the
// interim root-only gates are gone; its import lives there.
...
use iggy_common::wire_conversions::{
    ...
    partitioning_to_wire, permissions_to_wire, personal_access_tokens_from_wire, streams_from_wire,
    topics_from_wire, users_from_wire,
};
use iggy_common::{
    ...
    Identifier, IdentityInfo, IggyError, IggyMessageView, IggyTimestamp, PersonalAccessTokenInfo,
    PollMessages, PolledMessages, RESYNC_REQUIRED_PARTITION_SENTINEL, RawPersonalAccessToken,
    SendMessages, Stats, Stream, StreamDetails, TokenInfo, Topic, TopicDetails, TransportEndpoints,
    UserInfo, UserInfoDetails, Validatable,
};
...
use crate::responses::{
    ...
    build_raw_pat_reply, connected_client_to_response, resolve_stream_id, resolve_topic_id,
};
...
const MAX_HTTP_SESSIONS: usize = CLIENTS_TABLE_MAX / 2;
...
struct ClusterRoster {
    ...
    /// when no roster is configured.
    http_addr: SocketAddr,
    /// This node's other client listener ports for the same self-node
    /// synthesis (`None` = transport disabled; HTTP comes from `http_addr`).
...
impl RegistrationBarrier {
    ...
    fn enter(&self, key: &str) -> BarrierEntry<'_> {
...
struct HttpInner {
    ...
    registrations: RegistrationBarrier,
    roster: ClusterRoster,
...
impl HttpInner {
    ...
    fn forget_session(&self, session: &Rc<HttpSession>) {
...
/// hash -- a wrong one rejects as 400 `InvalidCredentials`.
...
/// write. On either rewrite's rejection (a malformed body, or a failed
/// current-password check) the gate is released without advancing, leaving the
/// id free.
...
fn read_local(
    ...
    match build_non_replicated_response(&state.shard, code, body, Some(identity.user_id))
        .map_err(ReadError::Rejected)?
    {
...
fn resolve_gate_stream(state: &HttpInner, stream_id: &WireIdentifier) -> Option<usize> {
    state
        .shard
        .plane
        .metadata()
...
/// single-node path: this node's own listener ports, with the HTTP port taken
/// from the live bound address and disabled transports reporting 0.
fn self_node_metadata(roster: &ClusterRoster) -> ClusterMetadata {
    ClusterMetadata {
        ...
        nodes: vec![ClusterNode {
            ...
            endpoints: TransportEndpoints::new(
                roster.self_ports.tcp.unwrap_or(0),
                roster.self_ports.quic.unwrap_or(0),
                roster.http_addr.port(),
                roster.self_ports.websocket.unwrap_or(0),
            ),
...
mod tests {
    ...
    // -- session table + registration barrier --
