...
mod tests {
    ...
    /// Consensus context the eviction shard stamps, spelled out so the
    /// assertion does not read it back from the object under test. Replica is
    /// 2 rather than 1 so all three differ: equal values would hide a builder
    /// that stamps the cluster into the replica field, or the zeroed fallback.
    const EVICTION_CLUSTER: u128 = 1;
    const EVICTION_VIEW: u32 = 0;
    const EVICTION_REPLICA: u8 = 2;
    ...
    async fn evict(
        ...
        // Literals, not `metadata_consensus(shard)`: reading the context back
        // from the source the builder read it from cannot catch a mis-mapped
        // field. `test_shard(.., 1, 3, ..)` seeds cluster 1, view 0, replica 1.
        assert_eq!(
            (header.cluster, header.view, header.replica),
            (EVICTION_CLUSTER, EVICTION_VIEW, EVICTION_REPLICA),
    ...
    async fn eviction_must_carry_the_typed_reason_and_the_live_consensus_context() {
        let bus = SpyBus::default();
        let shard = Rc::new(test_shard(&bus, EVICTION_REPLICA, 3, FIRST_BOOT));
