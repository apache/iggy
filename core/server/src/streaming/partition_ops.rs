...
pub async fn poll_messages(
    ...
    if strategy.kind == PollingKind::Timestamp {
        return poll_messages_by_timestamp(partition_store, namespace, value, count).await;
    }
    ...
    let batches = get_messages_by_offset(partition_store, namespace, start_offset, count).await?;
    Ok((metadata, batches))
}
...
pub async fn get_messages_by_offset(
    ...
    if is_journal_empty {
        return load_messages_from_disk(partition_store, namespace, start_offset, count).await;
    }
    ...
    if disk_count > 0 {
        let disk_messages =
            load_messages_from_disk(partition_store, namespace, start_offset, disk_count).await?;
...
async fn poll_messages_by_timestamp(
    ...
    if is_journal_empty {
        let batches =
            load_messages_from_disk_by_timestamp(partition_store, namespace, timestamp, count)
                .await?;
    ...
    let disk_messages =
        load_messages_from_disk_by_timestamp(partition_store, namespace, timestamp, count).await?;
