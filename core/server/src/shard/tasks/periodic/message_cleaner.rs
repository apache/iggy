...
async fn handle_expired_segments(
    ...
    let expired_segment_offsets: Vec<u64> = {
        ...
        let Some(partition_data) = store.get(&ns) else {
...
async fn handle_oldest_segments(
    ...
    let oldest_segment_offset = {
        let store = shard.partition_store.borrow();
        store.get(&ns).and_then(|partition_data| {
...
async fn delete_segments(
    ...
    // Extract segments and storages to delete from partition_store
    let (stats, segments_to_delete, mut storages_to_delete) = {
        let mut store = shard.partition_store.borrow_mut();
        let Some(partition_data) = store.get_mut(&ns) else {
            return Ok(DeletedSegments::default());
        };
