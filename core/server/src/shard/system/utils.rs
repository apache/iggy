...
use iggy_common::{Consumer, ConsumerKind, IdKind, Identifier, IggyError};
use std::sync::Arc;
...
fn resolve_stream_inner(
    ...
    let id = match stream_id.kind {
        IdKind::Numeric => {
            let sid = stream_id
                .get_u32_value()
                .map_err(|_| IggyError::StreamIdNotFound(stream_id.clone()))?
                as usize;
    ...
    Ok(ResolvedStream(id))
}

fn resolve_topic_id_inner(
    m: &InnerMetadata,
    stream_id: usize,
    topic_id: &Identifier,
) -> Option<usize> {
    ...
    match topic_id.kind {
        IdKind::Numeric => {
            let tid = topic_id.get_u32_value().ok()? as usize;
...
fn resolve_consumer_group_id_inner(
    m: &InnerMetadata,
    stream_id: usize,
    topic_id: usize,
    group_id: &Identifier,
    ...
    match group_id.kind {
        IdKind::Numeric => {
            let gid = group_id.get_u32_value().ok()? as usize;
