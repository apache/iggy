...
impl IggyShard {
    ...
    async fn handle_request(&self, request: ShardRequest) -> Result<ShardResponse, IggyError> {
        ...
        match request.payload {
            ...
            // Metadata operations - delegate to metadata coordinator
            ...
            | ShardRequestPayload::DeleteTopic { .. } => {
                self.handle_metadata_request(request.payload).await
            }
        }
    }
    ...
    pub(crate) async fn handle_event(&self, event: ShardEvent) -> Result<(), IggyError> {
        match event {
            ...
            ShardEvent::CreatedStream2 { id, stream } => {
                // Use the metadata operation's apply_event method
                use self::metadata::MetadataOperation;
                use self::metadata::operations::CreateStreamOp;

                let op = CreateStreamOp::new(stream.root().name().to_string());
                let event = self::metadata::operations::StreamCreatedEvent { id, stream };
                op.apply_event(self, &event)?;
                Ok(())
            }
            ...
            ShardEvent::CreatedTopic2 { stream_id, topic } => {
                // Use the metadata operation's apply_event method
                use self::metadata::MetadataOperation;
                use self::metadata::operations::CreateTopicOp;

                let op = CreateTopicOp::new(
                    stream_id.clone(),
                    topic.root().name().to_string(),
                    topic.root().message_expiry(),
                    topic.root().compression_algorithm(),
                    topic.root().max_topic_size(),
                    Some(topic.root().replication_factor()),
                );
                let event = self::metadata::operations::TopicCreatedEvent { stream_id, topic };
                op.apply_event(self, &event)?;
                Ok(())
            }
