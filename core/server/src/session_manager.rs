...
impl SessionManager {
    ...
    pub fn attach_consumer_session(
        ...
        if !attachment.session_is_valid() {
    ...
    /// `StaleClient` when the parent epoch has ended, or `TransientNotAccepted`
    /// when a group assignment change requires reattachment.
