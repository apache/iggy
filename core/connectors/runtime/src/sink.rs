...
async fn process_messages(
    ...
    // First non-zero status across the runs. A batch is committed only when the
    // sink took every run, so one refusal condemns the whole batch.
    let mut consume_result = 0i32;
...
struct SinkBatchTiming {
    ...
    /// First non-zero `iggy_sink_consume` status across the runs, or 0 when the
    /// sink took every one of them.
    consume_result: i32,
}
