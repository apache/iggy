...
use compio::io::{AsyncRead, AsyncWriteExt, BufReader};
...
impl TransportConn for TcpTransportConn {
    ...
    async fn run(self, ctx: ActorContext) {
        ...
        // A `BufReader` hands its buffer to the inner read and cannot take
        // it back if the `fill_buf` future is dropped mid-read; the next
        // call on it then panics. Invariant that keeps that unreachable:
        // the reader task owns the `BufReader` and nothing else can call
        // it, so the one path that drops a pending fill (the registry
        // force-cancelling this task at `close_peer_timeout` or drain)
        // drops the `BufReader` along with it.
        let reader_handle = match replica_read {
            Some((capacity, stats)) if capacity > 0 => compio::runtime::spawn(reader_loop(
                BufReader::with_capacity(capacity, CountingRead::new(read_half, Rc::clone(&stats))),
                in_tx,
                max_message_size,
                Some(stats),
                label,
            ...
            Some((_, stats)) => compio::runtime::spawn(reader_loop(
                CountingRead::new(read_half, Rc::clone(&stats)),
                in_tx,
                max_message_size,
                Some(stats),
                label,
...
/// source.
///
/// Sits under the [`BufReader`], so it sees socket reads rather than the
/// buffer hits the framing layer makes against the buffer. Wraps the
/// replica read half even when read-ahead is off, so the unbuffered arm
/// of an A/B run reports the same ratio as the buffered ones.
...
impl<R: AsyncRead> AsyncRead for CountingRead<R> {
    ...
    async fn read<B: IoBufMut>(&mut self, buf: B) -> BufResult<usize, B> {
        ...
        // cost the same submit and completion as a full read.
...
/// [`BufReader`] wrapping [`CountingRead`] while the client plane hands
