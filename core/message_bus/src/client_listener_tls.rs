...
//! TCP-TLS listener for consensus-protocol SDK clients.
//!
...
//! ([`crate::installer::install_client_tls_stream`]) so a slow or
...
//! Post-handshake the rustls `UnbufferedConnection` state machine holds
//! per-record sequence numbers, key schedule, and write buffers tied to
//! the local task, with no dupable plaintext fd. Both reasons compose
//! into invariant I5's TLS-family clause (no fd-delegation), formalised
//! in P9-T13.
//!
//! The TLS plane structurally cannot preserve `Frozen<MESSAGE_ALIGN>`
//! ownership: rustls's `WriteTraffic::encrypt` copies plaintext bytes
//! into the outbound ciphertext buffer, so I8 (zero-copy on plaintext
//! TCP) is preserved on the plaintext plane only. P9-T13 formalises the
//! amendment.
...
/// [`crate::installer::install_client_tls_stream`]).
#[allow(clippy::future_not_send)]
pub async fn run(
    ...
    info!(
        "Consensus TCP-TLS client listener accepting on {:?}",
    ...
    loop {
        futures::select! {
            () = token.wait().fuse() => {
                debug!("Consensus TCP-TLS client listener shutting down");
                break;
            }
            result = listener.accept().fuse() => {
                match result {
                    ...
                    Err(e) => {
                        error!("Consensus TCP-TLS client listener accept failed: {e}");
                    }
                }
            }
        }
