...
//! WSS (WebSocket-over-TLS) listener for consensus-protocol SDK clients.
//!
...
//! HTTP-Upgrade handshake (with the `iggy.consensus.v1` subprotocol
//! enforcement) run INSIDE the install path
//! ([`crate::installer::install_client_wss_stream`] →
...
//! dupable plaintext fd. Both reasons compose into invariant I5's
//! TLS-family clause (no fd-delegation), formalised in P9-T13.
//!
//! Like the TCP-TLS plane, WSS structurally cannot preserve
//! `Frozen<MESSAGE_ALIGN>` ownership: rustls's `WriteTraffic::encrypt`
//! copies plaintext bytes into the outbound ciphertext buffer, and the
//! WS frame builder concatenates header + body into a per-frame scratch
//! before that copy. I8 (zero-copy on plaintext TCP) is preserved on
//! the plaintext plane only. P9-T13 formalises the amendment.
//!
//! Subprotocol enforcement lives inside the `run` body of
//! [`crate::transports::wss::WssTransportConn`] — the in-tree HTTP
//! upgrade dance returns HTTP 400 on missing or wrong
//! `Sec-WebSocket-Protocol`. The install path stays thin and does not
//! duplicate the check.
...
///
/// Identical pattern to [`crate::client_listener_tls::bind`]: a single
...
pub async fn run(
    ...
    info!(
        "Consensus WSS client listener accepting on {:?}",
    ...
    loop {
        futures::select! {
            () = token.wait().fuse() => {
                debug!("Consensus WSS client listener shutting down");
                break;
            }
            result = listener.accept().fuse() => {
                match result {
                    ...
                    Err(e) => {
                        error!("Consensus WSS client listener accept failed: {e}");
                    }
                }
            }
        }
