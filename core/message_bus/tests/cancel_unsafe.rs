...
//!
//! This test is the load-bearing artefact behind invariant I14 in
//! `core/message_bus/CLAUDE.md` and the rationale for the hand-rolled
//! TLS driver (`transports/tls/driver.rs`). It empirically demonstrates
//! that `compio`'s `io_uring` `read*` futures are NOT cancel-safe:
//! dropping a mid-flight `read_exact` future loses bytes that the
//! kernel has already DMA'd into the caller-owned buffer.
//!
//! Sources:
...
//!
//! The TLS driver structurally avoids the trap by never `select!`-ing
//! over a TCP read on a still-alive connection (driver.rs:54-60). If a
...
//! verdict, the IO model assumptions in CLAUDE.md must be re-audited.
//!
//! Mechanics (mirrors `/tmp/compio_cancel_repro/src/main.rs`):
//!   1. Loopback TCP pair.
...
async fn read_future_drop_loses_already_dma_bytes() {
    ...
    let err = read_res.expect_err(
        "phase 2 returned Ok(()): all 256 bytes preserved across the dropped read_exact. \
         compio cancel semantics changed. Re-audit CLAUDE.md I14 and the TLS driver \
         design before relaxing the no-`select!`-over-reads invariant.",
    );
    assert_eq!(
        err.kind(),
