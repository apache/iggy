...
//! shared `Arc<rustls::ServerConfig>` to `install_client_wss_stream`,
//! which drives both the rustls handshake and the WS HTTP-Upgrade
//! (with `iggy.consensus.v1` subprotocol enforcement) on its own task
//! before installing reader / writer tasks. The handler echoes a Reply
//! back via `bus.send_to_client`, the client reads the Reply.
//!
...
use message_bus::client_listener_wss::{bind, run};
